...
# as a subgroup of $\operatorname{PSL}_2(\mathbb{C})$
...
#
# As stated, this definition defines the twisted pair group up to conjugacy in $\operatorname{PSL}_2(\mathbb{C})$,
...
#
# Suppose we have any embedding of the group into $\operatorname{PSL}_2(K)$
...
# In general, an algebraic isomorphism between subgroups of $\operatorname{PSL}_2(\mathbb{C})$ need not preserve
# matrix traces,
# but trace-preserving isomorphisms are exactly the ones that are induced by inner automorphisms of
# $\operatorname{PSL}_2(\mathbb{C})$, up to a single choice of sign.
...
# $\operatorname{PSL}_2(\mathbb{C})$.

# NOTES
# - with this strategy, "geometric entities" can be points, specifically the orbit of the attractor of the $u$ transformation.
# - even with the structure rule, there's a whole aesthetic dimension on how colors combine together
#   in the final image that we won't touch on here at all.
...
# the trace of every element in the subgroup of $\operatorname{PSL}_2(\mathbb{C})$
...
# $$
# and any field $K$ with an embedding of the twisted pair group into $\operatorname{PSL}_2(K)$ must
...
# Suppose we want to embed this group into $\operatorname{PSL}_2(F)$ for some field
...
# of the twisted pair group in $\operatorname{PSL}(2, 16)$,
...
# The order of $\operatorname{PSL}(2, 16)$ itself is 4080,
# which means the permutation group we obtain here is $\operatorname{PSL}(2, 16)$
# itself.
